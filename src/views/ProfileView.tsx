...
export const ProfileView: React.FC<ProfileViewProps> = ({ onNavigate }) => {
  ...
  // Sync profile details when user is available
  useEffect(() => {
    if (user) {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        ...
        if (signUpError) {
          if (signUpError.message.toLowerCase().includes('rate limit') || signUpError.status === 429) {
            ...
            if (!fallbackSignInError) return;
            throw new Error('Security Throttling: Please check your email or wait a moment.');
          }
          throw signUpError;
        }

        alert(`Verification required. Please check ${email}.`);
      } else {
      ...
      if (msg.includes('Failed to fetch')) msg = 'Connection lost. Check your Atelier settings.';
      setError(msg);
    } finally {
      setLoading(false);
    }
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {[
          ...
          <button
            ...
            className={`flex items-center gap-4 p-6 glass-card rounded-[2.5rem] border-white/5 hover:border-luxury-gold/30 transition-all text-left group ${item.red ? 'hover:border-red-500/30' : ''}`}
          >
            <div className={`p-4 rounded-2xl ${item.red ? 'bg-red-500/10 text-red-500' : 'bg-luxury-gold/10 text-luxury-gold'}`}>
              <item.icon className="w-6 h-6" />
            </div>
            <div>
              <h4 className={`font-black text-gray-900 dark:text-white group-hover:text-luxury-gold transition-colors text-sm uppercase tracking-wider ${item.red ? 'group-hover:text-red-500' : ''}`}>{item.label}</h4>
